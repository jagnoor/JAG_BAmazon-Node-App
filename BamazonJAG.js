...
function inquireForPurchase() {
    ...
    inquirer.prompt([
        ...
        const quantityDesired = answers.Quantity;
        const IDDesired = answers.ID;
...
function purchaseFromDatabase(ID, quantityNeeded) {
    ...
    connection.query('SELECT * FROM Products WHERE ItemID = ' + ID, function(error, response) {
        ...
        if (quantityNeeded <= response[0].StockQuantity) {
            //calculate cost
            const totalCost = response[0].Price * quantityNeeded;
            //inform user
        ...
        displayStuffandOtherthings();//recursive shopping is best shopping! Shop till you drop!
    });
